...
- **Powerups**: every player signs up with **3 powerups** (duplicates allowed) from ✅ Auto-Correct (base points even if wrong), 💣 Bomb (pick a victim, −10 at reveal, **only fires if the bomber answers correctly**, wasted otherwise, no defending), ✂️ 50/50 (snips two wrong MC answers, MC-only — greyed elsewhere), 🎲 Double or Nothing (doubles what you win, burns on nothing), 🛢️ Oil Slick (5s of un-tappable dripping oil on a rival's phone, immediate), ☢️ Tactical Nuke (−20 every other player, −10 self, **nobody wins the question's points**, only detonates if the nuker answers correctly, **max 1 held** — `puCanHold`/`maxHold` enforced at join, claim and `foldPuPicks`; resolved LAST in `applyPowerups` so it claws back auto/double grants and restates bomb maths). **Question rounds only** (MC/typed/slider), playable from the 3-2-1 countdown to the whistle, **one play per player per question**, consumed win or lose. The projector announces oil slicks live (dripping banner + splat) and an armed nuke immediately (fixed flashing `qb-nuke-alert` strip + air-raid siren — imperative on `<body>` like the banners, so the poll can't blink it); bombs stay secret until the reveal. Plays are own-key writes under `qb-power-<puEpoch>` (`puEpoch` minted at load, NOT `answerEpoch`, so countdown plays work); the host resolves everything in `applyPowerups` (inside scoring, after normal points, so double includes bonuses and the bomb maths — `targetWon − 10 = net` — is spelled out in `puEvents` for the reveal). Every **25 points climbed** (`PU_EVERY`; `puHi` rungs, monotonic — sinking and re-crossing earns nothing) banks a pick. Rungs are derived from the score, so changing `PU_EVERY` mid-game (or meeting a player who joined before powerups existed) would retroactively mint a backlog — 4 picks at once for someone on 100. `p.puLadder` records the spacing a player's rungs were counted under, and `refreshPuHi` **rebases** when it differs: picks genuinely *owed* carry over, everything below their current score counts as already collected. The phone writes the choice to `qb-pupick/<id>` and the host folds it in (`foldPuPicks`) when a slot (max 3) is free. A game reset restores `puChosen` (the sign-up picks). The projector shows held icons beside names; the admin sees armed plays live.
