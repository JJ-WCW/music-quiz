...
- **Wagers**: set `isWager` + `wagerClue` on **any** question type. Projector shows the clue first; players bet up to `MAX_BET` (100) into a kitty, **scores may go negative**. Players who don't fancy it tap **No bet**, which writes a `{amount:0, pass:true}` ante — that's what makes `antesDecided()` reach the player count so the host knows to stop waiting. Host closes betting, question plays normally, **the winner takes the whole kitty**. If **exactly one player bet and that player wins**, the house matches their stake on top of the pot (`loneBetBonus()` — otherwise a solo bettor just wins their own points back, which made betting alone pointless); `state.betMatched` carries the matched amount to the reveal messaging. Betting is optional; everyone keeps their buzzer.
