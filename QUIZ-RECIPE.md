...
> **Do NOT create wager rounds.** The betting/kitty mechanic is dormant — players now use **powerups** instead (chosen at sign-up in the app; they need nothing in the question JSON). Leave `isWager` off every question unless the user explicitly asks for a wager round; if they do, the old flow still works: `"isWager": true, "wagerClue": "hint shown before the question", "wagerImage": ""`, bettors-only kitty, roll-over, lone-bet house match.
